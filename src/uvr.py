...
sys.path.append('/home/jupyter/whisper')
...
class UVRModel:
    def __init__(self, model_path=None):
        ...
        # model_path = model_path or os.path.join(project_dir, '../denoise_models/UVR-MDX-NET-Inst_Main.onnx')
    ...
    def separate_offline(self, mixed_audio, sample_rate=44100):
        ...
        for d in range(bands_n, 0, -1):
            ...
            if d == bands_n:  # high-end band
                ...
                x_wave[d] = librosa.core.resample(x_wave[d + 1], orig_sr=UVR_CONFIG['band'][d + 1]['sr'],
                                                  target_sr=bp['sr'],
...
if __name__ == '__main__':
    model = UVRModel()
    audio, sample_rate = AudioReader.read_wav_file('../../denoise_in/Extreme.Job.2019.KOREAN.1080p.BluRay.x265-VXT.wav')
    instrument, vocal = model.separate_offline(audio, sample_rate)

    print("sample_rate : ", sample_rate)
    print("vocal : ", vocal)

    out_path = "../../denoise_out"

    sf.write(os.path.join(out_path, "no_vocals.wav"), instrument, sample_rate)
    sf.write(os.path.join(out_path, "vocals.wav"), vocal, sample_rate)
